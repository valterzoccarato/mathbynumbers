...
async function loadConstants() {
  ...
  if (state.constants.length === 0) {
    els.digitDisplay.innerHTML = '<span class="loading">Nessuna costante trovata.</span>';
    return;
  }
...
async function switchConstant(id) {
  ...
  els.totalDigits.textContent = m.total_digits.toLocaleString("it-IT");
  els.totalChunks.textContent = m.n_chunks;
  els.generatedAt.textContent = new Date(m.generated_at).toLocaleString("it-IT");
...
function buildChunkList(id, manifest) {
  ...
  for (let i = 1; i <= manifest.n_chunks; i++) {
    ...
    text.textContent = `Blocco ${i} (${startM.toFixed(0)}\u2013${endM.toFixed(0)}M)`;
...
function renderSingle(highlight = null) {
  ...
  if (loadedNums.length === 0) {
    els.digitDisplay.innerHTML = '<span class="loading">Seleziona uno o pi\u00f9 blocchi dalla lista a sinistra per vederne le cifre.</span>';
    els.viewerLabel.textContent = "Nessun blocco selezionato";
    return;
  }

  els.viewerLabel.textContent = `${loadedNums.length} blocco/hi caricato/i: ${loadedNums.join(", ")}`;

  let html = "";
  for (const n of loadedNums) {
    const digits = cs.loadedChunks.get(n);
    html += `<div class="chunk-block" data-chunk-block="${n}">`;
    html += `<div class="chunk-block-header">Blocco ${n}</div>`;
...
function updateModeUI() {
  ...
  els.viewToggleBtn.textContent = isJoint ? "Vista singola" : "Vista congiunta";
  els.viewToggleBtn.classList.toggle("active", isJoint);
  els.searchTitle.textContent = isJoint ? "Cerca in tutte le costanti" : "Cerca un pattern";
  els.searchHint.textContent = isJoint
    ? "Cerca lo stesso pattern in tutte le costanti contemporaneamente: i risultati sono colorati per costante."
    : "Cerca su TUTTE le cifre precalcolate, anche quelle non caricate a video.";
...
async function buildJointColumns() {
  ...
  for (const c of state.constants) {
    ...
    display.innerHTML = '<span class="loading">Caricamento\u2026</span>';
    ...
    for (let i = 1; i <= m.n_chunks; i++) {
      ...
      opt.textContent = `Blocco ${i} (${startM.toFixed(0)}\u2013${endM.toFixed(0)}M)`;
...
async function runSearch() {
  ...
  if (!/^\d+$/.test(pattern)) {
    els.searchStatus.textContent = "Inserisci solo cifre (0-9).";
    return;
  }
  els.searchStatus.textContent = "Ricerca in corso\u2026";
  els.results.innerHTML = "";
...
async function runSearchSingle(pattern) {
  ...
  if (data.count_returned === 0) {
    els.searchStatus.textContent = `Nessuna occorrenza di "${pattern}" trovata nelle ${cs.manifest.total_digits.toLocaleString("it-IT")} cifre decimali precalcolate.`;
    return;
  }

  els.searchStatus.textContent =
    `${data.count_returned} occorrenza/e mostrata/e` +
    (data.truncated ? " (ce ne sono altre, raffina la ricerca)" : "") + ":";

  els.results.innerHTML = "";
  data.results.forEach((r) => {
    const div = document.createElement("div");
    div.className = "result-item";
    div.innerHTML = `posizione <b>${r.position.toLocaleString("it-IT")}</b> (dopo la virgola)`;
...
async function runSearchJoint(pattern) {
  ...
  outcomes.forEach(({ constant, data }) => {
    ...
    groupHeader.innerHTML = `<span class="const-tag" style="color:${color}">${constant.symbol} ${constant.label}</span> \u2014 ${data.count_returned} occorrenza/e` +
      (data.truncated ? " (ce ne sono altre)" : "");
    ...
    if (data.count_returned === 0) {
      ...
      div.textContent = "nessuna occorrenza";
      els.results.appendChild(div);
      return;
    }
    ...
    data.results.forEach((r) => {
      ...
      div.className = "result-item joint";
      div.style.setProperty("--col-color", color);
      div.innerHTML = `posizione <b>${r.position.toLocaleString("it-IT")}</b>`;
  ...
  els.searchStatus.innerHTML = totalFound > 0
    ? `Trovate per costante: ${summaryParts.join(" &middot; ")}. Clicca un risultato per vederlo nella colonna corrispondente.`
    : `Nessuna occorrenza di "${pattern}" trovata in nessuna delle costanti (${summaryParts.join(" &middot; ")}).`;
}
